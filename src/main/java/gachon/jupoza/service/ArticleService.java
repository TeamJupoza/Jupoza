...
import gachon.jupoza.repository.ArticleRepository;
import gachon.jupoza.repository.ArticleStockRepository;
...
//            article.get().setMyStockList(null);
    ...
    public Map<String,Object> findOneArticle(Long articleId) {
        try {
            ...
            for (int i = 0; i < 5; i++) {
                StockEntityList.add(myPortfolioService.getStock(article.get().getMyStockList().get(i).getStockId()));
            }
